...
if args.problem == "classification":
    ...
    if loss == "default":
        loss = "squared_error"
sklearn_dual_est.set_params(loss=loss)


if args.print_params:
    print("scikit-learn")
    pprint(sklearn_dual_est.get_params())

    for libname in ["lightgbm", "xgboost", "catboost"]:
        if getattr(args, libname):
            print(libname)
            est = get_equivalent_estimator(
                sklearn_dual_est, lib=libname, n_classes=args.n_classes
            )
...
def one_run(n_threads, n_samples):
    ...
    print("Fitting a sklearn_dual model...")
    tic = time()
    est = sklearn_dual.base.clone(sklearn_dual_est)

    with threadpool_limits(n_threads, user_api="openmp"):
        est.fit(X_train, y_train, sample_weight=sample_weight_train)
        sklearn_dual_fit_duration = time() - tic
        tic = time()
        sklearn_dual_score = est.score(X_test, y_test)
        sklearn_dual_score_duration = time() - tic
    print("score: {:.4f}".format(sklearn_dual_score))
    print("fit duration: {:.3f}s,".format(sklearn_dual_fit_duration))
    print("score duration: {:.3f}s,".format(sklearn_dual_score_duration))
    ...
    return (
        sklearn_dual_score,
        sklearn_dual_fit_duration,
        sklearn_dual_score_duration,
        lightgbm_score,
...
sklearn_dual_scores = []
sklearn_dual_fit_durations = []
sklearn_dual_score_durations = []
lightgbm_scores = []
...
for n_threads in n_threads_list:
    ...
    for scores, score in (
        (sklearn_dual_scores, sklearn_dual_score),
        (sklearn_dual_fit_durations, sklearn_dual_fit_duration),
        (sklearn_dual_score_durations, sklearn_dual_score_duration),
...
if args.plot or args.plot_filename:
    ...
    label = f"sklearn_dual {sklearn_dual.__version__}"
    axs[0].plot(n_threads_list, sklearn_dual_fit_durations, label=label)
    axs[1].plot(n_threads_list, sklearn_dual_score_durations, label=label)
