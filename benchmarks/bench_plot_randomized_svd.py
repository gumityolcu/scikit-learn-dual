...
def bench_a(X, dataset_name, power_iter, n_oversamples, n_comps):
    ...
    for pi in power_iter:
        for pm in ["none", "LU", "QR"]:
            print("n_iter = %d on sklearn_dual - %s" % (pi, pm))
...
if __name__ == "__main__":
    ...
    for dataset_name in datasets:
        ...
        if X is None:
            continue
        print(
            " >>>>>> Benching sklearn_dual and fbpca on %s %d x %d"
    ...
    print(" >>>>>> Benching sklearn_dual and fbpca default configurations")
