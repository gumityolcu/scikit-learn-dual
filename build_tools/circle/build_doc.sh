...
affected_doc_paths() {
    ...
    sklearn_dual_files=$(echo "$files" | grep '^sklearn_dual/')
    if [ -n "$sklearn_dual_files" ]
    then
        grep -hlR -f<(echo "$sklearn_dual_files" | sed 's/^/scikit-learn\/blob\/[a-z0-9]*\//') doc/_build/html/stable/modules/generated | cut -d/ -f5-
    fi
}
